...
export default function CalendarView({ appData }) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const dayTasks = appData.tasks.filter((t) => isTaskActiveOnDate(t, date));
  ...
  return (
    <Card style={{ marginBottom: 20 }} delay={0.15}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 2, marginBottom: 4 }}>
        {DAY_NAMES.map((d) => (
          <div key={d} style={{
            textAlign: "center", fontSize: 10, fontWeight: 700, color: C.steel,
            ...
            >
              ...
              {hasTasks && (
                <div style={{ display: "flex", gap: 2, marginTop: 3 }}>
                  {dayTaskList.slice(0, 3).map((_, j) => (
                    <div key={j} style={{
                      width: 4, height: 4, borderRadius: "50%",
                      background: isSelected ? "rgba(255,255,255,0.7)" : C.navy,
                    }} />
                  ))}
                  {dayTaskList.length > 3 && (
                    <div style={{
                      width: 4, height: 4, borderRadius: "50%",
                      background: isSelected ? "rgba(255,255,255,0.4)" : C.steel,
                    }} />
                  )}
                </div>
              )}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          ...
          {selectedDate === todayStr && (
            <span style={{
              ...
              color: C.navy, background: C.ice, padding: "4px 10px", borderRadius: 50,
            }}>Today</span>
          )}
        </div>

        {selectedTasks.length === 0 ? (
          <p style={{ color: C.steel, fontSize: 14, padding: "8px 0" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {selectedTasks.map((task) => {
              ...
              const allAssignees = task.assignedTo?.length
              ...
              return (
                <div key={task.id} style={{
                  ...
                  background: completed ? "rgba(118,138,150,0.06)" : "rgba(223,235,246,0.25)",
                  border: `1px solid ${completed ? C.border : C.borderLight}`,
                  opacity: completed ? 0.6 : 1,
                  transition: "all 0.2s",
                }}>
                  <div style={{
                    width: 6, height: 6, borderRadius: "50%", flexShrink: 0,
                    background: completed ? C.steel : C.navy,
                  }} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{
                      fontWeight: 600, fontSize: 14, color: C.dark,
                    ...
                    <div style={{ display: "flex", gap: 10, marginTop: 2, flexWrap: "wrap" }}>
                      ...
                      {task.timeDue && !task.dueConfig?.time && (
                        <span style={{ fontSize: 11, color: C.steel }}>{task.timeDue}</span>
                      )}
                      {task.points > 0 && (
