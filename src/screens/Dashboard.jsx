...
export default function Dashboard({ appData, setAppData, onAddMember, onCreateTask, onEditTask, requestedView, clearRequestedView, pendingPreview, clearPendingPreview }) {
  ...
  const handleComplete = async (taskId) => {
    ...
    if (!task) return;
    const completion = { id: uid(), taskId, userId: appData.currentUserId, timestamp: new Date().toISOString(), pointsEarned: task.points || 0 };
    const updatedTasks = appData.tasks.map((t) => t.id === taskId ? { ...t, lastCompleted: completion.timestamp, status: "completed" } : t);
    const updatedUsers = appData.users.map((u) => u.id === appData.currentUserId ? { ...u, pointBalance: (u.pointBalance || 0) + (task.points || 0) } : u);
    ...
    const isAssignedToMe = assignees.includes(appData.currentUserId);
    const completedForOthers = assignees.filter((id) => id !== appData.currentUserId);

    // Notify the task creator (if different from completer) — from Divvy
    if (task.createdBy && task.createdBy !== appData.currentUserId) {
      sendSystemNotification(task.createdBy, "completion", `${completerName} completed the task: "${task.name}"`, { taskId, completedBy: appData.currentUserId });
    }

    // Notify all assigned users (except the completer) — from Divvy
    for (const userId of completedForOthers) {
      sendSystemNotification(userId, "completion", `${completerName} completed your task: "${task.name}"`, { taskId, completedBy: appData.currentUserId });
    }

    // If someone completed a task assigned to someone else, show the "thank you / trade" overlay to the assigned person
    if (!isAssignedToMe && completedForOthers.length > 0) {
      // For the assigned users, they'll see an actionable notification when they check
      // For the completer, show a toast
      showToast(`You completed "${task.name}" for ${completedForOthers.map(getUserName).join(", ")}`);
    }
  };
...
Interpret relative dates like "next Saturday" into actual dates. If the command mentions telling someone or asking someone to do something, it's a task assigned to that person. If it's about thanking or appreciating someone, it's a recognition. If it's a reminder for the user themselves, it's a reminder.`,
    ...
    return (
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: compact ? 10 : 14, padding: compact ? "10px 0" : "14px 0" }}>
          {/* Complete button — large tap target */}
          <button onClick={() => done ? handleUncomplete(task.id) : handleComplete(task.id)}
            style={{
              all: "unset", cursor: "pointer", flexShrink: 0,
              display: "flex", alignItems: "center", justifyContent: "center",
              width: compact ? 32 : 38, height: compact ? 32 : 38,
              borderRadius: 10,
              background: done ? "rgba(68,87,109,0.08)" : "rgba(68,87,109,0.04)",
              border: `1.5px solid ${done ? C.navy : "rgba(68,87,109,0.25)"}`,
              transition: "all 0.2s",
            }}>
            <CheckCircleIcon done={done} size={compact ? 18 : 22} />
          </button>
          ...
          {/* Explicit complete/undo label */}
          {!compact && (
            <button onClick={() => done ? handleUncomplete(task.id) : handleComplete(task.id)}
              style={{
                ...btnBase, padding: "6px 14px", fontSize: 11, borderRadius: 8, flexShrink: 0,
                background: done ? "rgba(68,87,109,0.06)" : C.gradientPrimary,
                color: done ? C.steel : C.white,
                boxShadow: done ? "none" : "0 2px 8px rgba(41,53,60,0.2)",
              }}>
              {done ? "Undo" : "Complete"}
            </button>
          )}
        ...
        {showActions && (
          <div style={{ display: "flex", gap: 8, padding: "0 0 8px 52px", animation: "fadeUp 0.2s ease both" }}>
  ...
  if (view === "memberDetail" && selectedMember) {
    ...
    return (
      <PageShell narrow topNav>
        <button onClick={() => setView("members")} style={{ ...btnGhost, padding: "4px 0", marginBottom: 16 }}>&larr; Back</button>
  ...
  if (view === "tasks") {
    ...
    return (
      <PageShell narrow topNav>
        <Header title="Tasks" onBack={() => setView("hub")} />
